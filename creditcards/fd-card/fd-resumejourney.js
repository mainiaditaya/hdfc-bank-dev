...
import { restAPICall, fetchJsonResponse } from '../../common/makeRestAPI.js';
import { OCCUPATION_MAP, GENDER_MAP, JOURNEY_NAME } from './constant.js';
    ...
    if (returnState) {
       ...
       if (returnJourneyInfo.currentFormContext !== undefined) {
            ...
            if (applRefNumber !== undefined) {
                ...
                let savedTime = payload?.journey?.[0]?.journeyStateInfo?.[0]?.timeinfo;
                const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
                let timeStamp = savedTime.substring(8, 10) + " " + months[parseInt(savedTime.substring(5, 7)) - 1] + " " + savedTime.substring(0, 4) + ", " + tConvert(savedTime.substring(11, 19));
                ...
                let sessionTimeStamp = `Your previous session was saved on ${timeStamp}`;
...
const prefillResumeJourneyData = async (resumeJourneyResponse, globals) => {
    ...
    const { personalDetails, addressDetails, employmentDetails, employeeAssistance } = reviewDetailsView;
...
const getResumeJourneyJsonObject = () => {
    return RESUME_JOURNEY_JSON_OBJECT;
}
